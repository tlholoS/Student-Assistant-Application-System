...
class ApplicationFormScreen extends StatelessWidget {
                              ...
                              child: Column(
                                children: [
                                  PremiumTextField(
                                    ...
                                    controller: TextEditingController(text: viewModel.yearOfStudy)..selection = TextSelection.fromPosition(TextPosition(offset: viewModel.yearOfStudy.length)),
                                  ...
                                  DropdownButtonFormField<String>(
                                    ...
                                    items: viewModel.academicLevels.map((level) => DropdownMenuItem(value: level, child: Text(level))).toList(),
                              ...
                              child: Column(
                                children: [
                                  ...
                                  DropdownButtonFormField<ModuleModel>(
                                    ...
                                    items: viewModel.availableModules.map((m) => DropdownMenuItem(value: m, child: Text('${m.moduleCode} - ${m.moduleName}'))).toList(),
                                    onChanged: viewModel.setModule1,
                                  ),
                                  const SizedBox(height: 20),
                                  _buildDropdownLabel('Secondary Module (Optional)'),
                                  DropdownButtonFormField<ModuleModel>(
                                    decoration: const InputDecoration(prefixIcon: Icon(Icons.add_circle_outline_rounded)),
  ...
  Widget _buildFormSection(BuildContext context, {required String title, required IconData icon, required Widget child}) {
    return Column(
  ...
  Widget _buildUploadArea(BuildContext context, ApplicationFormViewModel viewModel) {
    final hasFile = viewModel.supportingDocument != null;
    return InkWell(
      onTap: viewModel.pickDocument,
      borderRadius: BorderRadius.circular(20),
      child: Container(
    ...
    return Container(
      ...
      child: CheckboxListTile(
        title: Text(
          ...
          style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.grey.shade700)
        ),
        ...
        onPressed: viewModel.isLoading
            ...
            : () async {
                ...
                if (authViewModel.currentUserData != null) {
                  final success = await viewModel.submitApplication(authViewModel.currentUserData!);
                  if (success && context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Application Submitted Successfully! 🚀', style: GoogleFonts.outfit(fontWeight: FontWeight.w600)),
